...
export default class OrderForm extends Component {
  ...
  handleUserInput = (e) => {
    ...
    const fieldFormData = this.state.formData;
    fieldFormData[name] = value;
    this.setState({formData: fieldFormData},
  ...
  validateField(fieldName, value) {
    ...
    switch(fieldName) {
      case 'name':
        ...
        fieldValidationErrors.name = nameValid ? '' : 'Поле должно быть заполнено';
        break;
      case 'email':
      ...
      case 'code-country':
          ...
          fieldValidationErrors.codeCountry = codeCountryValid ? '' : '3 цифры';
          break;
      case 'mobile-number':
          const mobileNumberValid = fieldValue.match(/\b\d{7}\b/);
          fieldValidationErrors.mobileNumber = mobileNumberValid ? '' : '7 цифр';
          break;
      case 'address':
          const addressValid = fieldValue.length > 0;
          fieldValidationErrors.address = addressValid ? '' : 'Поле должно быть заполнено';
          break;
      default:
          break;
    }
          ...
          <div className="column column_s-2 column_m-3 column_l-7">
                <div className="form__head">
                  ...
                  <div className="form__item">
                      ...
                      />
                      <span className="form__error-message">{this.state.formErrors.name}</span>
                  </div>
                  {/*Электронная почта*/}
                  <div className="form__item">
                      <input 
                        ...
                        onChange={this.handleUserInput}
                      />
                      <span className="form__error-message">{this.state.formErrors.email}</span>
                  </div>
                  {/*Телефон*/}
                    <div className="form__phone">
                    <div className="form__phone-wrap">
                      <input type="text" className="input input_phone-pre" defaultValue="+7"/>
                    </div>
                    <div className="form__phone-wrap">
                      <input 
                        type="text" 
                        name="code-country" 
                        className={`input input_phone-code ${this.errorClass(this.state.formErrors.codeCountry)}`} 
                        placeholder="Код" 
                        maxLength="3" 
                        required
                        onChange={this.handleUserInput}
                      />
                      <span className="form__error-message">{this.state.formErrors.codeCountry}</span>
                    </div> 
                    <div className="form__phone-wrap_grow">
                      <input 
                        type="text" 
                        name="mobile-number" 
                        className={`input input_phone-number ${this.errorClass(this.state.formErrors.mobileNumber)}`} 
                        placeholder="Номер" 
                        maxLength="7" 
                        required
                        onChange={this.handleUserInput}
                      />
                      <span className="form__error-message">{this.state.formErrors.mobileNumber}</span>
                    </div>
                    </div>
              </div>
              ...
              <div className="form__section">
                ...
                <div className="form__radio">
                  <label className="form__label popup-order__form-label" htmlFor="delivery-1">
                   <input 
                    type="radio" 
                    id="delivery-1" 
                    name="delivery" 
                    ...
                    const fieldFormData = this.state.formData;
                    delete fieldFormData["delivery"];
                    delete fieldFormData["city"];
                    delete fieldFormData["address"];

                    this.setState({ checkedDelivery: "pickup", formErrors: fieldValidationErrors, formData: fieldFormData }, this.validateForm);
                    }}
                  />
                   <span className="radio-button popup-order__form-button">Самовывоз</span>
                  </label>
                  <label className="form__label popup-order__form-label" htmlFor="delivery-2">
                    <input 
                     type="radio" 
                     id="delivery-2" 
                     name="delivery" 
                     ...
                     onClick={e => {
                      const fieldValidationErrors = this.state.formErrors;
                      fieldValidationErrors.address = null;
                      const fieldFormData = this.state.formData;
                      fieldFormData["delivery"] = "delivery";
                      this.setState({ checkedDelivery: "delivery", formErrors: fieldValidationErrors, formData: fieldFormData }, this.validateForm);
                    }}
                     defaultChecked
                    />
                   <span className="radio-button popup-order__form-button">Доставка</span>
                 </label>
                </div>
              </div>
              {/*Адрес*/}
              {this.state.checkedDelivery !== "pickup" && (
                  <div className="form__section">
                    <div className="form__delivery-address">
                      ...
                      <select name="city" className="select select_arrows" 
                       onChange={e => {
                        const fieldFormData = this.state.formData;
                        fieldFormData["city"] = e.target.value;
                    ...
                    <div className="form__item">
                      <textarea 
                        name="address" 
                        className={`textarea ${this.errorClass(this.state.formErrors.address)}`} 
                        maxLength="300" 
                        required
                        onChange={this.handleUserInput}
                      >
                      </textarea>
                      <span className="form__error-message">{this.state.formErrors.address}</span>
                    </div>
                  </div>
              )}
              {/*Оплата*/}
              <div className="form__items-section">
                ...
                <div className="form__item">
                  ...
                  <input type="radio" name="pay" id="online" className="radio" defaultValue="online" defaultChecked
                    onChange={this.handleUserInput}
                  />
                ...
                <div className="form__item">
                  ...
                  <input type="radio" name="pay" id="cash" className="radio" value="cash"
                    onChange={this.handleUserInput}
                  />
                  <span className="radio-box"></span>
                ...
                <div className="form__item">
                  ...
                  <input type="radio" name="pay" id="card" className="radio" value="card"
                    onChange={this.handleUserInput}
                  />
                  <span className="radio-box"></span>
              ...
              <div className="form__items-section">
                ...
                <div className="form__item">
                  ...
                  <input type="checkbox" name="sms" id="sms" className="checkbox"

                  onClick={e => {
                      const fieldFormData = this.state.formData;
                      fieldFormData["sms"] = e.target.checked;
                      this.setState({formData: fieldFormData });
                    }}
                  />
                  <span className="checkbox-box"></span>
                  <span className="checkbox-text">Хочу получать SMS уведомления</span>
                  </label>
                </div>
              </div>
